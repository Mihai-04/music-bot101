  ...
  if (command === "play" || command === "p" || command === "") {
    ...
    if (!voiceChannel)
      return message.channel.send({
        embed: {
          color: "RED",
          description: "Be In A Voice Channel First Yo !"
        }
      });
    ...
    if (!permissions.has("CONNECT")) {
      return message.channel.send({
        embed: {
          color: "RED",
          description:
            "I Need A **`CONNECT`** Permission To Proceed !"
        }
      });
    }
    if (!permissions.has("SPEAK")) {
      return message.channel.send({
        embed: {
          color: "RED",
          description: "I Need A **`SPEAK`** Permission To Proceed !"
        }
      });
    }
    if (!url || !searchString)
      return message.channel.send({
        embed: {
          color: "RED",
          description: "Please Type The Link/Title To Play Music !"
        }
      });
  ...
  if (command === "search" || command === "sc") {
    ...
    if (!url || !searchString)
      return message.channel.send({
        embed: {
          color: "RED",
          description: "Please Type The Link/Title To Search Music !"
        }
      });
    if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
      const playlist = await youtube.getPlaylist(url);
      ...
      try {
        ...
        try {
          ...
          let embedPlay = new MessageEmbed()
            ...
            .setFooter(
              "Please Choose One Of The Following 10 Results, This Embed Will Be Auto Deleted In 15 Seconds !"
            );
    ...
    if (!message.member.voice.channel)
      return message.channel.send({
        embed: {
          color: "RED",
          description:
            "You Need To Be In A Voice Channel To Set A Volume !"
        }
      });
    if (!serverQueue)
      return message.channel.send({
